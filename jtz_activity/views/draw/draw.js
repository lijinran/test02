 ...
 */
require(["vue", "common", "apiData", "extend", "dialog", "timeouter", "validate", 'config', "vue-marquee", 'vue-navmenu', "area"], function(Vue, app, api, ext, dialog, timeouter, validate, c) {
    let WIN = window,
        DOC = document;
    ...
    ext.wxIntercept((barData, wxLogin) => {
        ...
        let vm = new Vue({
            ...
            data: {
                ...
                thirdLink: '', // 跳转链接
                thirdTitle: '金天子随手拍活动正在火热开展中', // 标题
                thirdId: '', // hotMessge id
                phone: '',
                code: '',
                contacts: '',
                contactsPhone: '',
            ...
            methods: {
                ...
                goToLink() {
                    ...
                    api.addHotMsgRecord(vm.thirdId, vm.thirdTitle, wxLogin.userToken, barData.province, barData.city, barData.area).done(
                        res => {
                            console.log(res);
                        }
                    )
                    ...
                    setTimeout(function() {
                        window.location.href = vm.thirdLink;
                    }, 500)
                        ...
                        data => {
                            ...
                            switch (data.code) {
                                case 200:
                                    ...
                                    //跳转地址
                                    vm.thirdLink = edata.hotMessage.jumpUrl || 0;
                                    vm.thirdTitle = edata.hotMessage.title;
                                    vm.thirdId = edata.hotMessage.id;
                                    // 存储活动信息
                                    app.storeValue('ActivityInfo', JSON.stringify(edata.activity), 'session')

                                    // 重新取时间
                                    getRollTime = app.getValue('getRollTime', 'local') || 0;
                                    srcollList = app.getValue('srcollList', 'local') || 0;
                                    // 服务器的时间大于本地时间则取数据
                                        ...
                                        case 200:
                                            if (eres == null || eres.win == 0) {
                                                ...
                                                setTimeout(function() {
                                                    ...
                                                    vm.prizelogoUrl = eres.logoUrl;
                                                    vm.prizeType = eres.prizeType;
                                ...
                                switch (data.code) {
                                    case 200:
                                        if (!data.data.noticeFlag) { // 提醒
                                            ...
                                            this.onlyOne = true;
                                            this.isOpacity = true;
                                            $('.animationPublic').css({'-webkit-animation-play-state': 'running!important','animation-play-state': 'running!important'});
                                            setTimeout(function() {
                ...
                PromptFun: function() { //确定不再提醒
                    ...
                    this.isOpacity = true;
                    $('.animationPublic').css({'-webkit-animation-play-state': 'running!important','animation-play-state': 'running!important'});
                   // $('.animationPublic').style.animationPlayState="running";
                ...
                registerShare: () => {
                    ...
                    ext.WX.wxs.onMenuShareTimeline({
                        ...
                        link: location.origin + th.shareUrl, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                    ...
                    ext.WX.wxs.onMenuShareAppMessage({
                        ...
                        link: location.origin + th.shareUrl, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                        imgUrl: location.origin + th.shareImg, // 分享图标
